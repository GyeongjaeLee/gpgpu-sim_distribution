...
Usage examples:
  ...
  python run_accelsim.py \
      ...
      --routing baseline min_adaptive near_min_adaptive near_min_random fixed_min \
      --near-min-k 2 \
      --near-min-p 0.0 1.0
...
def main() -> None:
    ...
    ap.add_argument("--near-min-k", nargs="+", type=int, metavar="K",
                    default=[2], help="Near-min routing budget k (default: 2)")
    ap.add_argument("--near-min-p", nargs="+", type=float, metavar="P",
                    default=[1.0], help="Near-min penalty p values (default: 1.0)")
    ...
    # Build routing key list dynamically based on combinations
    routing_keys: list = []
